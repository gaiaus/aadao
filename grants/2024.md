...
* **Margined Finance**: Aims to extend its vault framework to support delta-neutral liquidity provisioning (LPing). This innovation allows users to deposit into a vault that LPs into a specific pool while simultaneously creating a hedge for the LP position’s value. The system significantly lowers LPing risks by reducing exposure to both realized and impermanent loss. Grant Goals:
